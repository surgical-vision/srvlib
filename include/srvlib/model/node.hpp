...
// measured coordinates are in meters but we always work in mm.
  ...
  */
  ...
  public:
    ...
    */
    ...
    void SetSelf(Node::Ptr self){ self_ = self; }
    ...
    */
    virtual void LoadData(ci::JsonTree &tree, Node::Ptr parent, Node::Ptr self, const std::string &root_dir, size_t &idx) = 0;
    ...
    */
    virtual glm::mat4 GetWorldTransform(const glm::mat4 &base_frame_pose) const = 0;

    /**
    * Get the transform between the this node and the root node.
    * @return The transform in a 4x4 float matrix.
    */
    virtual glm::mat4 GetRelativeTransformToRoot() const = 0;
    ...
    * @return The transform in a 4x4 float matrix.
    */
    virtual glm::mat4 GetRelativeTransformToChild(const int child_idx) const = 0;
    ...
    */
    virtual glm::mat4 GetTransformFromParent() const = 0;
    ...
    */
    virtual void ComputeJacobianForPoint(const glm::mat4 &world_transform, const glm::vec3 &point, const int target_frame_idx, std::vector<glm::vec3> &jacobian) const;
    ...
    */
    const Node::Ptr GetParent() const { return parent_; }
    ...
    */
    bool HasMesh() const { return mesh_->getNumVertices() > 0; }
    ...
    */
    virtual glm::mat4 GetRelativeTransformToNodeByIdx(const int target_idx) const = 0;

    /**
    * Get the axis around which or along which the node transforms
    * @return The transformation axis.
    */
    virtual glm::vec3 GetAxis() const = 0;
  ...
  protected:
    ...
    */
    ...
    ci::gl::VboMeshRef	mesh_; /**< VBO to store the model for faster drawing. */
  ...
  */
  ...
  public:
    ...
    */
    ...
    static DHNode::Ptr Create(){ Node::Ptr self(new DHNode()); self->SetSelf(self); return boost::dynamic_pointer_cast<DHNode>(self); }
    ...
    * @param[in] idx The index of this node (useful for accessing nodes).
    */
    virtual void LoadData(ci::JsonTree &tree, Node::Ptr parent, Node::Ptr self, const std::string &root_dir, size_t &idx);
    ...
    * @return The world transform in a 4x4 float matrix.
    */
    virtual glm::mat4 GetWorldTransform(const glm::mat4 &base_frame_pose) const;

    /**
    * Get the transform between the this node and the parent node using the DH transforms.
    * @return The transform in a 4x4 float matrix.
    */
    virtual glm::mat4 GetRelativeTransformToRoot() const;
    ...
    */
    ...
    void ComputeJacobianForHead(const glm::mat4 &world_transform, const glm::vec3 &point, std::vector<glm::vec3> &jacobian) const;
    void ComputeJacobianForClasperYaw(const glm::mat4 &world_transform, const glm::vec3 &point, std::vector<glm::vec3> &jacobian) const;
    void ComputeJacobianForClasperRotate(const glm::mat4 &world_transform, const glm::vec3 &point, const int target_frame_index, std::vector<glm::vec3> &jacobian) const;

    /**
    * Get the transform between the this node and one of its child nodes using the DH transforms.
    * @return The transform in a 4x4 float matrix.
    */
    virtual glm::mat4 GetRelativeTransformToChild(const int child_idx) const;
    ...
    */
    virtual glm::mat4 GetTransformFromParent() const;
    ...
    * @return The relative transform from caller node to target node. Identity transform if not connected nodes.
    */
    virtual glm::mat4 GetRelativeTransformToNodeByIdx(const int target_idx) const;

    /**
    * Get the axis around which or along which the node transforms
    * @return The transformation axis.
    */
    virtual glm::vec3 GetAxis() const { if (type_ == Alternative) return alt_axis_; else return glm::vec3(0,0,1); }

    /**
    * Get the derivative of the transform from parent to child w.r.t the transform parameter.
    * @return The jacobian matrix.
    */
    virtual glm::mat4 GetDerivativeTransfromFromParent() const;

    void UpdateThisNode(const float &update) { update_ += update; }
    void SetUpdateThisNode(const float &update) { update_ = update; }
  ...
  protected:
    ...
    */
    void createFixedTransform(const glm::vec3 &axis, const float rads, glm::mat4 &output) const;
    ...
    */
    ...
    glm::vec3 alt_axis_; /**< Hack axis for daVinci instruments. */
