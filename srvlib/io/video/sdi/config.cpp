...
BMDConfig::BMDConfig() :
  m_deckLinkLeftIndex(-1),
  m_deckLinkRightIndex(-1),
...
bool BMDConfig::ParseArguments(int argc,  char** argv)
{
...
#ifdef __linux__
  while ((ch = getopt(argc, argv, "f:d:?h3c:s:v:a:m:n:p:t:")) != -1)
    {
      switch (ch)
	{
	case 'd':
	  m_deckLinkLeftIndex = atoi(optarg);

	case 'f':
	  m_deckLinkRightIndex = atoi(optarg);
	  break;
  ...
  if (m_deckLinkLeftIndex < 0)
    {
      fprintf(stderr, "You must select a device for the left stream\n");
      DisplayUsage(1);
    }

  if (m_deckLinkRightIndex < 0)
    {
      fprintf(stderr, "You must select a device for the right stream\n");
      DisplayUsage(1);
    }
  ...
  IDeckLink* deckLink = GetDeckLink(m_deckLinkLeftIndex);
  if (deckLink != NULL)
    {
      if (m_displayModeIndex != -1)
	{
...
void BMDConfig::DisplayUsage(int status)
{
  ...
  fprintf(stderr,
	  "Usage: Capture -d <left device id> -f <right device id> -m <mode id> [OPTIONS]\n"
	  "\n"
	  "    -d <left device id>:\n"
	  "    -f <right device id>:\n"
	  );
  ...
  while (deckLinkIterator->Next(&deckLink) == S_OK)
...
#elif defined _WIN32
      deckLinkName = new char[100];
      int wslen = MultiByteToWideChar(CP_ACP, 0, (const char*)deckLinkName, strlen((const char*)deckLinkName), 0, 0);
      BSTR bstr = SysAllocStringLen(0, wslen);
      MultiByteToWideChar(CP_ACP, 0, (const char*)deckLinkName, strlen((const char*)deckLinkName), bstr, wslen);
      result = deckLink->GetModelName(&bstr);
      SysFreeString(bstr);
      delete[] deckLinkName;
      deckLinkName = 0x0;
#endif
      if (result == S_OK)
	{
	  fprintf(stderr,
		  ...
		  deckLinkCount,
		  deckLinkName,
		  deckLinkCount == m_deckLinkLeftIndex ? " (selected)" : ""
		  );

	  free(deckLinkName);
	}

      if (deckLinkCount == m_deckLinkLeftIndex)
...
#elif defined _WIN32
      BOOL formatDetectionSupported_ = formatDetectionSupported;
      result = deckLinkAttributes->GetFlag(BMDDeckLinkSupportsInputFormatDetection, &formatDetectionSupported_);
#endif     
  ...
  while (displayModeIterator->Next(&displayModeUsage) == S_OK)
    {
      ...
      result = displayModeUsage->GetName(&bstr);
      SysFreeString(bstr);
